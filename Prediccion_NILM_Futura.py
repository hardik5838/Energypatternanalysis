...
def entrenar_modelo_y_predecir(df_historico, df_futuro):
    """
    Entrenar un Random Forest Regressor usando variables de temperatura y lags,
    y predice el consumo total futuro.
    """
    # Preparar datos de entrenamiento
    features = ['temp_avg_c', 'dia_semana', 'dia_mes', 'mes', 'es_finde', 'consumo_lag_1', 'consumo_lag_7']
...
def aplicar_nilm_futuro(df_prediccion):
    ...
    df_prediccion['iluminacion_predicho'] = np.where(
        ...
        df_prediccion['consumo_total_predicho'] * 0.10  # Uso reducido fines de semana
    )
...
def show_forecast_nilm_page(df_consumo_hist, df_clima_hist, df_clima_futuro):
    st.title("Predicción de Consumo y NILM a Futuro")
    st.subheader("Pronóstico de Carga (Lags & Temperatura) + Desagregación (HVAC/Iluminación)")
    st.markdown("---")
    
    with st.spinner('Procesando datos históricos y entrenando el modelo de regresión...'):
