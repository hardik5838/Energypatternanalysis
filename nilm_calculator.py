...
def simulate_physics_strategy(df_sim, config):
    """
    Executes Level Tier 1: Rule-Based Heuristic Pre-cooling (Glider Theory)
    using the calibrated physical variables straight from the Oasis digital twin.
    """
    df = df_sim.copy()
    ...
    # Spain Mandated maximum comfort constraint
    t_comfort_max = 27.0 if 'hvac_setpoint' not in config else min(27.0, config['hvac_setpoint'] + 4.0) [cite: 288]
    hours_of_p1 = 4.0 # 10:00 - 14:00 [cite: 289]
    
    # Derive building-specific rise rate (R_drift) from envelope UA and estimated capacitance [cite: 290]
    # If capacitance isn't explicitly defined, we use the rule-of-thumb scaled mass relation
    building_capacitance = config.get('hvac_ua', 1200.0) / 7.0 
    
    # Characteristic temperature rise rate in °C/hr based on thermal mass [cite: 290]
    r_drift = max(0.5, min(3.0, (config.get('hvac_q_int', 10.0) + 5.0) / (building_capacitance + 0.1))) 
    
    # Formula (1): Calculate dynamic launch target ceiling boundary 
    target_launch_temp = t_comfort_max - (hours_of_p1 * r_drift) [cite: 284]
    target_launch_temp = max(target_launch_temp, 19.0) # Pre-cool Safety Floor Constraint [cite: 293]
    
    # 2. RUN BI-DIRECTIONAL SHIFT LOGIC ACROSS REAL LOADS
    df['sim_optimized'] = df['sim_total']
    
    # MASK 1: The P1 Peak Shutdown (10:00 - 14:00) -> Freeze the Compressor [cite: 289, 292]
    p1_mask = (df['hora'] >= 10) & (df['hora'] < 14)
    
    # Isolate the exact active thermal energy load generated by your NILM simulation
    energy_removed_from_p1 = df.loc[p1_mask, 'sim_therm'].sum()
    
    # Coordinated Mass Load Shedding: drop the cooling profile during peak [cite: 292, 473]
    df.loc[p1_mask, 'sim_optimized'] -= df.loc[p1_mask, 'sim_therm']
    
    # MASK 2: The P3 Pre-cool Thermal Charging Window (04:00 - 09:00) 
    p3_mask = (df['hora'] >= 4) & (df['hora'] < 9)
    
    # Quantify thermal mass parameters to figure out required compensation energy
    ...
    # Electrical energy required to artificially sink space temperatures into structural mass
    ...
    # Hard-cap the pre-cooling charging rate by the actual max capacity of the infrastructure [cite: 327]
    ...
    # Valley Filling: safely inject the shifted pre-cooling load into the cheap P3 window [cite: 472]
    df.loc[p3_mask, 'sim_optimized'] += hourly_pre_cool_injection
    
    # 3. COMPENSATE FOR UNEXPECTED REBOUND OVERLAPS
    # Make sure we enforce the infrastructure's Contracted Power Threshold cap 
    if 'hvac_cap_max' in config:
        df['sim_optimized'] = np.clip(df['sim_optimized'], 0, config['hvac_cap_max'] + config.get('base_kw', 20.0))
    
    # 4. CALCULATE TRUE FINANCIAL OPTIMIZED PERFORMANCE DELIVERABLES
    df['cost_optimized'] = df['sim_optimized'] * df['price_kwh']
    
    # Diagnostic state tracking telemetry (Useful if you want to inspect variables in st.write)
    st.session_state['runtime_r_drift'] = r_drift
    st.session_state['runtime_t_launch'] = target_launch_temp
